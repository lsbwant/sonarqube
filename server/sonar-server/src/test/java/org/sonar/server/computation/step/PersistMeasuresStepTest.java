...
public class PersistMeasuresStepTest extends BaseStepTest {
  ...
  public void setUp() {
    ...
    metric = new MetricDto().setKey(METRIC_KEY).setEnabled(true);
    ...
    sut = new PersistMeasuresStep(dbClient, ruleCache, metricCache, persistedComponentsRefCache);
  }
  ...
  private ComponentDto addComponent(int ref, String key){
    ...
    session.commit();
    persistedComponentsRefCache.addComponent(ref, new PersistedComponentsRefCache.PersistedComponent(componentDto.getId(), key, componentDto.uuid()));
    return componentDto;
  }
